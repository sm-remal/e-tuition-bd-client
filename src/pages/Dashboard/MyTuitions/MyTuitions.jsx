...
const MyTuitions = () => {
  ...
  // Fetch user tuitions from backend
  useEffect(() => {
  ...
  const handleEdit = (id) => console.log('Edit tuition:', id);
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-gray-500">
                {searchTerm ? "Try adjusting your search or filter criteria" :
                 filter==='all' ? "You haven't posted any tuitions yet" : `No ${filter} tuitions available`}
              </p>
            </div>
          ) : (
                    ...
                    <tr key={tuition._id} className={`hover:bg-blue-50 transition-colors ${index%2===0?'bg-white':'bg-gray-50'}`}>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <button onClick={() => handleEdit(tuition._id)} className="p-2 text-green-600 hover:bg-green-100 rounded-lg transition-all"><Edit className="w-5 h-5" /></button>
